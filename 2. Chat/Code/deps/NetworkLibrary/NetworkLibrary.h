...
extern uint32_t g_TotalNumberOfPacketsSent;
extern uint32_t g_TotalNumberOfPacketsReceived;
extern long long g_TotalTimeSendingPackets; //microsecond
extern long long g_TotalTimeReceivingPackets; //microsecond
...
public:
	...
	int SendPacketToServer(Packet* packet)
	{
		...
		g_TotalTimeSendingPackets += duration;
		++g_TotalNumberOfPacketsSent;
		return sending;
	}
	...
	int ReceiveDataFromServer(char* receiveBuffer)
	{
		...
		std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
		g_TotalTimeReceivingPackets += duration;
...
public:
	...
	bool AcceptNewClient(uint32_t id)
	{
		...
		// add client
		m_ClientSessions.insert({ id, client_socket });
...
protected:
	...
	virtual void ReceiveDataFromServer()
	{
		...
		while (i < data_length)
		{
			++g_TotalNumberOfPacketsReceived;
...
protected:
	...
	// data buffer
	char m_NetworkData[MAX_PACKET_SIZE] = { 0 };
};
